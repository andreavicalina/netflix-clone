...
<html>
  ...
  <div class="container-fluid">
    ...
    <div class="container-fluid mt-3">
      ...
      <?php
        $query = mysqli_query($koneksi, "SELECT * FROM series");
        foreach ($query as $q):
      ?>
        <div class=" col-3 mr-4 mb-5 gmbr-utama">
          <a type="button" data-toggle="modal" data-target="#detsrs<?= $q['id_series']; ?>">
            <img src="<?= "../admin/cover/".$q['poster_srs']?>"  alt="">
          </a>
        </div>
        <div class="modal fade bd-example-modal-lg" id="detsrs<?= $q['id_series']; ?>" role="dialog">
          <div class="modal-dialog modal-dialog-centered modal-lg" style="background: #221f1f;">
            <div class="modal-content" style="background: #221f1f;">
              ...
              <?php
                $id_series = $q['id_series'];
                $detsrs = mysqli_query($koneksi, "SELECT * FROM series a, genre b, batas_usia c WHERE id_series = $id_series AND a.id_genre = b.id_genre AND a.id_batas_usia = c.id_batas_usia");
                          foreach ($detsrs as $srs) {
              ?>
                  <div class="card mb-3" style="background: #221f1f; color: white;">
                    <img class="card-img-top" src="<?= "../admin/cover/".$q['poster_srs']?>" alt="Card image cap">
                    <div class="card-body">
                      <ul class="modal__btns row">
                        <li>
                          ...
                          <a href="add_favorite.php?id=<?=$id?>&id_cont=<?=$srs['id_series']?>" class="modal__btn" style="color: white;">
                      ...
                      <ul class="modal__ratings row">
                        <p class="modal__pg ml-3"><?= $srs['batas_usia'] ?></p>
                        <p class="modal__season ml-3"><strong><?= $srs['judul_srs'] ?></strong></p>
                      </ul>
                      <ul class="modal__categories row">
                        <p class="modal__category ml-3"><?= $srs['alur_srs'] ?></p>
                      </ul>
                      <p class="card-text"><?= $srs['deskripsi_srs'] ?></p>
                      <hr style="border: 1px solid grey;">
                      <?php
                        $id_series = $q['id_series'];
                        $eps = mysqli_query($koneksi, "SELECT * FROM detail_eps WHERE id_series = $id_series");
                        foreach ($eps as $eps):
                  ?>
                        <h6 style="color: white;">Season <?=$eps['id_season']?> episode <?=$eps['id_eps']?></h6>
                        <p style="color: grey;"><?= $eps['durasi_eps'] ?></p>
                        <p style="color: white;"><?=$eps['deskripsi_eps']?></p><br>
                  <?php
                        endforeach;
                  ?>
                    </div>
